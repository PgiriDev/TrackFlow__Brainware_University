...
@section('content')
    ...
    <!-- Colorful Glassmorphism Page Background - Green/Emerald Theme -->
    <div class="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        <div
            class="absolute inset-0 bg-gradient-to-br from-emerald-100 via-green-50 to-teal-100 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900">
        </div>
        <div
            class="absolute top-0 left-0 w-[600px] h-[600px] bg-gradient-to-br from-emerald-300/40 to-green-400/40 rounded-full blur-3xl -translate-x-1/3 -translate-y-1/3 dark:from-emerald-600/10 dark:to-green-700/10">
        </div>
        <div
            class="absolute bottom-0 right-0 w-[600px] h-[600px] bg-gradient-to-br from-teal-300/40 to-cyan-400/40 rounded-full blur-3xl translate-x-1/3 translate-y-1/3 dark:from-teal-600/10 dark:to-cyan-700/10">
        </div>
        <div
            class="absolute top-1/2 left-1/2 w-[500px] h-[500px] bg-gradient-to-br from-green-300/30 to-emerald-400/30 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 dark:from-green-600/10 dark:to-emerald-700/10">
        </div>
        <div
            class="absolute top-1/4 right-1/4 w-[400px] h-[400px] bg-gradient-to-br from-lime-300/30 to-green-400/30 rounded-full blur-3xl dark:from-lime-600/10 dark:to-green-700/10">
        </div>
        <div
            class="absolute bottom-1/4 left-1/4 w-[400px] h-[400px] bg-gradient-to-br from-teal-300/30 to-emerald-400/30 rounded-full blur-3xl dark:from-teal-600/10 dark:to-emerald-700/10">
        </div>
    </div>

    <script>
        // Global variable to store selected transaction IDs
        window.selectedTransactionIds = [];
    </script>
    <div class="animate-fade-in relative" x-data="{
                                                                                            showFilters: false,
                                                                                            ...
                                                                                            toggleSelectAll() {
                                                                                                ...
                                                                                                window.selectedTransactionIds = [...this.selectedTransactions];
                                                                                            },
                                                                                            toggleTransaction(id) {
                                                                                                const idStr = String(id);
                                                                                                const index = this.selectedTransactions.indexOf(idStr);
                                                                                                if (index > -1) {
                                                                                                    this.selectedTransactions.splice(index, 1);
                                                                                                } else {
                                                                                                    this.selectedTransactions.push(idStr);
                                                                                                }
                                                                                                const checkboxes = document.querySelectorAll('#transactionsTableBody input[type=checkbox]');
                                                                                                this.selectAll = this.selectedTransactions.length === checkboxes.length && checkboxes.length > 0;
                                                                                                window.selectedTransactionIds = [...this.selectedTransactions];
                                                                                            },
                                                                                            clearSelection() {
                                                                                                this.selectedTransactions = [];
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            ...
            <div class="mt-4 sm:mt-0 flex items-center gap-3">
                <button onclick="openScheduleModal()"
                    class="inline-flex items-center px-5 py-3 border-2 border-amber-500 text-amber-600 dark:text-amber-400 font-medium rounded-lg transition-colors hover:bg-amber-50 dark:hover:bg-amber-900/20">
        ...
        <!-- Filters & Search -->
        <div
            class="bg-white/40 dark:bg-gray-800/50 backdrop-blur-xl rounded-xl shadow-lg border border-white/50 dark:border-gray-700/50 p-4 lg:p-6 mb-6 card-shadow">
            ...
            <div x-show="showFilters" x-cloak x-transition
                ...
                <div class="sm:col-span-2 lg:col-span-4 flex items-center justify-between pt-4">
                    ...
                    <button id="applyFiltersBtn" onclick="applyFilters()"
                        ...
                        <i id="applyFiltersIcon" class="fas fa-check"></i>
        ...
        <div id="scheduledTransactionsSection" class="mb-6 hidden">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <i class="fas fa-calendar-alt text-amber-500"></i>
                    Scheduled Transactions
                    <span id="scheduledCount"
                        class="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-xs rounded-full">0</span>
                </h2>
                <button onclick="toggleScheduledSection()"
                    class="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                    <i id="scheduledToggleIcon" class="fas fa-chevron-up"></i>
                </button>
            </div>

            <div id="scheduledTransactionsContent"
                class="bg-white/40 dark:bg-gray-800/50 backdrop-blur-xl rounded-xl shadow-lg border border-amber-200/50 dark:border-amber-700/30 overflow-hidden">
                <!-- Desktop Table -->
                <div class="hidden lg:block overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-700/30">
                            <tr>
                                <th
                                    class="px-6 py-3 text-left text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                                    Scheduled Date</th>
                                <th
                                    class="px-6 py-3 text-left text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                                    Description</th>
                                <th
                                    ...
                                    Category</th>
                                <th
                                    class="px-6 py-3 text-right text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                                    Amount</th>
                                <th
                                    class="px-6 py-3 text-center text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                                    Status</th>
                                <th
                                    class="px-6 py-3 text-center text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wider">
                                    Actions</th>
                        ...
                        <tbody id="scheduledTransactionsTableBody"
                            class="divide-y divide-amber-100 dark:divide-amber-800/30">
                            <!-- Loading skeleton -->
                            <tr class="animate-pulse">
                                <td class="px-6 py-4">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-24"></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-40"></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-20"></div>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-20 ml-auto"></div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-16 mx-auto"></div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-24 mx-auto"></div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <!-- Mobile Cards -->
                <div id="scheduledTransactionsMobileCards" class="lg:hidden p-4 space-y-3">
                    <!-- Loading skeleton -->
                    <div class="p-4 animate-pulse bg-amber-50 dark:bg-amber-900/10 rounded-lg">
                        <div class="flex items-center space-x-4">
                            <div class="w-12 h-12 bg-amber-200 dark:bg-amber-700/30 rounded-lg"></div>
                            <div class="flex-1">
                                <div class="h-4 bg-amber-200 dark:bg-amber-700/30 rounded w-3/4 mb-2"></div>
                                <div class="h-3 bg-amber-200 dark:bg-amber-700/30 rounded w-1/2"></div>
                            </div>
                            <div class="h-5 bg-amber-200 dark:bg-amber-700/30 rounded w-20"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Transactions Table -->
        <div
            class="bg-white/40 dark:bg-gray-800/50 backdrop-blur-xl rounded-xl shadow-lg border border-white/50 dark:border-gray-700/50 overflow-hidden card-shadow">
            <!-- Desktop Table -->
            <div class="hidden lg:block overflow-x-auto">
                <table class="w-full">
            ...
            <form id="transactionForm" class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label
                                ...
                                <input type="radio" name="type" value="debit" checked class="hidden"
                                    onchange="filterCategoriesByType('expense')">
                            ...
                            <label
                                ...
                                <input type="radio" name="type" value="credit" class="hidden"
                                    onchange="filterCategoriesByType('income')">
                                <div class="text-center">
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button type="submit" id="saveTransactionBtn"
                        class="px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                        <i class="fas fa-save mr-2" id="saveTransactionIcon"></i><span id="saveTransactionText">Save
                            Transaction</span>
                    </button>
                </div>
            </form>
                ...
                <form id="quickCategoryForm" onsubmit="saveQuickCategory(event)">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            0 0 * * * cd /path/to/trackflow && php artisan transactions:send-reminders
                        </div>
                    </div>
    ...
    <div id="scheduleModal" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center p-4"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto"
            onclick="event.stopPropagation()">
            <div
                class="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gradient-to-r from-amber-500 to-orange-500">
                <h2 class="text-xl font-bold text-white flex items-center gap-2">
                    <i class="fas fa-calendar-alt"></i>
                    Schedule Transaction
                </h2>
                <button onclick="closeScheduleModal()" class="text-white/80 hover:text-white">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>

            <form id="scheduleForm" class="p-6 space-y-5">
                <!-- Transaction Type -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Transaction Type</label>
                    <div class="flex gap-4">
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="schedule_type" value="debit" class="sr-only peer" checked>
                            <div
                                class="p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-center peer-checked:border-red-500 peer-checked:bg-red-50 dark:peer-checked:bg-red-900/20 transition-all">
                                <i class="fas fa-arrow-down text-2xl text-red-500 mb-2"></i>
                                <p class="font-medium text-gray-700 dark:text-gray-300">Expense</p>
                            </div>
                        </label>
                        <label class="flex-1 cursor-pointer">
                            <input type="radio" name="schedule_type" value="credit" class="sr-only peer">
                            <div
                                class="p-4 border-2 border-gray-200 dark:border-gray-600 rounded-xl text-center peer-checked:border-green-500 peer-checked:bg-green-50 dark:peer-checked:bg-green-900/20 transition-all">
                                <i class="fas fa-arrow-up text-2xl text-green-500 mb-2"></i>
                                <p class="font-medium text-gray-700 dark:text-gray-300">Income</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        <i class="fas fa-calendar mr-1 text-amber-500"></i> Scheduled Date <span
                            class="text-red-500">*</span>
                    </label>
                    <input type="date" name="scheduled_date" id="scheduleDate" required
                        min="{{ date('Y-m-d', strtotime('+1 day')) }}"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                    <p class="text-xs text-gray-500 mt-1">Select a future date for this transaction</p>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span
                            class="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500 font-medium">{{ $currencySymbol }}</span>
                        <input type="number" name="schedule_amount" id="scheduleAmount" step="0.01" min="0.01" required
                            placeholder="0.00"
                            class="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="schedule_description" id="scheduleDescription" required
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Category</label>
                    <select name="schedule_category_id" id="scheduleCategory"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                ...
                <!-- Merchant -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Merchant/Payee</label>
                    <input type="text" name="schedule_merchant" id="scheduleMerchant"
                        placeholder="e.g., Landlord, Insurance Company"
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent">
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes</label>
                    <textarea name="schedule_notes" id="scheduleNotes" rows="2" placeholder="Add any additional notes..."
                        class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent resize-none"></textarea>
                </div>

                <!-- Info Box -->
                <div
                    class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/30 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        <i class="fas fa-info-circle text-amber-500 mt-0.5"></i>
                        <div class="text-sm text-amber-700 dark:text-amber-400">
                            <p class="font-medium mb-1">Email Notifications</p>
                            <ul class="list-disc list-inside text-xs space-y-1 text-amber-600 dark:text-amber-500">
                                <li>A confirmation email will be sent when you schedule this transaction</li>
                                <li>A reminder email will be sent at 12:00 AM on the scheduled date</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <button type="button" onclick="closeScheduleModal()"
                        class="px-6 py-3 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 font-medium rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                        Cancel
                    </button>
                    <button type="submit" id="scheduleSubmitBtn"
                        class="px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-medium rounded-lg transition-all shadow-lg hover:shadow-xl flex items-center gap-2">
                        <i id="scheduleSubmitIcon" class="fas fa-calendar-check"></i>
                        <span id="scheduleSubmitText">Schedule Transaction</span>
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        async function fetchTransactions(page = 1) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Apply category filter (compare by category ID)
                    if (filterCategory) {
                        transactions = transactions.filter(tx => String(tx.category_id) === String(filterCategory) || String(tx.category?.id) === String(filterCategory));
                    }
        ...
        function updatePaginationButtons() {
            ...
            paginationContainer.innerHTML = `
                                                                        <button onclick="goToPreviousPage()" 
                                                                            class="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        ...
        function clearTransactionSelection() {
            ...
            if (alpineEl && alpineEl._x_dataStack) {
                alpineEl._x_dataStack[0].selectedTransactions = [];
                alpineEl._x_dataStack[0].selectAll = false;
            }
            const checkboxes = document.querySelectorAll('#transactionsTableBody input[type=checkbox]');
            checkboxes.forEach(cb => cb.checked = false);
            const headerCheckbox = document.querySelector('thead input[type=checkbox]');
                                                                                                                                                                                                ...
                                                                                                                                                                                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" data-transaction-id="${tx.id}">
                                                                                                                                                                                                    <td class="px-6 py-4">
                                                                                                                                                                                                        <input type="checkbox" value="${tx.id}" 
                                                                                                                                                                                                               class="transaction-checkbox rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500"
                                                                                                                                                                                                               onchange="document.querySelector('[x-data]')._x_dataStack[0].toggleTransaction('${tx.id}')">
                                                                                                                                                                                                    </td>
        ...
        async function applyFilters() {
            ...
            const icon = document.getElementById('applyFiltersIcon');
            ...
            // Show Dynamic Island loading
            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.show('Filtering Transactions...');
            }
            ...
            try {
                ...
                if (typeof DynamicIsland !== 'undefined') {
                    DynamicIsland.success('Filters Applied!');
                }
        ...
        async function clearFilters() {
            ...
            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.show('Clearing Filters...');
            }

            await fetchTransactions();

            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.success('Filters Cleared!');
            }
        }
        ...
        function openAddTransactionModal() {
            ...
            document.querySelector('[name="date"]').value = new Date().toISOString().split('T')[0];
            // Default is expense (debit), so filter for expense categories
        ...
        document.getElementById('transactionForm').addEventListener('submit', async function (e) {
            ...
            const btnIcon = document.getElementById('saveTransactionIcon');
        ...
        async function bulkDelete() {
            const selectedIds = window.selectedTransactionIds || [];
            ...
            popupConfirm(
                ...
                async function () {
                    ...
                    if (failCount === 0) {
                        showNotification(`${successCount} transaction(s) deleted successfully`, 'success');
                    } else {
                        showNotification(`${successCount} deleted, ${failCount} failed`, 'warning');
                    }
                }
            );
        }
        ...
        // applyFilters and clearFilters functions are defined above - removed duplicate
        ...
        let scheduledTransactionsData = [];
        let scheduledSectionCollapsed = false;
        ...
        function openScheduleModal() {
            ...
            // Default is expense, so populate expense categories
            populateScheduleCategoryDropdown('expense');

            // Add event listeners for type change
            document.querySelectorAll('input[name="schedule_type"]').forEach(radio => {
                radio.addEventListener('change', function () {
                    const selectedType = this.value === 'debit' ? 'expense' : 'income';
                    populateScheduleCategoryDropdown(selectedType);
                });
            });
        }
        ...
        // Populate Schedule Category Dropdown based on type
        function populateScheduleCategoryDropdown(type = 'expense') {
            ...
            if (filteredCategories.length > 0) {
                filteredCategories.forEach(cat => {
                    ...
                    options += `<option value="${cat.id}" data-icon="${cat.icon || ''}" data-color="${cat.color || ''}">${cat.name}</option>`;
                });
            } else {
                options += `<option value="" disabled>No ${type} categories found</option>`;
            }
        ...
        function toggleScheduledSection() {
            const content = document.getElementById('scheduledTransactionsContent');
            const icon = document.getElementById('scheduledToggleIcon');

            scheduledSectionCollapsed = !scheduledSectionCollapsed;

            if (scheduledSectionCollapsed) {
        ...
        async function fetchScheduledTransactions() {
            try {
                const response = await fetch('/transactions/scheduled/list', {
                    headers: { 'Accept': 'application/json' }
                });

                if (response.ok) {
                    const data = await response.json();
                    scheduledTransactionsData = data.data;

                    const section = document.getElementById('scheduledTransactionsSection');
                    ...
                    if (scheduledTransactionsData.length > 0) {
                        ...
                        countBadge.textContent = data.meta.pending;
                        displayScheduledTransactions(scheduledTransactionsData, data.meta.currencySymbol);
                    } else {
        ...
        function displayScheduledTransactions(transactions, currencySymbol) {
            const tbody = document.getElementById('scheduledTransactionsTableBody');
            const mobileContainer = document.getElementById('scheduledTransactionsMobileCards');

            const pendingTransactions = transactions.filter(tx => tx.status === 'pending');

            if (pendingTransactions.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">No scheduled transactions</td></tr>';
                mobileContainer.innerHTML = '<div class="p-4 text-center text-gray-500 dark:text-gray-400">No scheduled transactions</div>';
                return;
            }
            ...
            tbody.innerHTML = pendingTransactions.map(tx => {
                ...
                const isDueToday = tx.is_due_today;
                const statusClass = isOverdue ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' :
                    isDueToday ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' :
                        'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400';
                const statusText = isOverdue ? 'Overdue' : isDueToday ? 'Due Today' : 'Pending';
                                                ...
                                                <tr class="hover:bg-amber-50/50 dark:hover:bg-amber-900/10 transition-colors">
                                                    <td class="px-6 py-4">
                                                        <div class="flex items-center gap-2">
                                                            ...
                                                            <span class="text-sm font-medium text-gray-900 dark:text-white">${formatDate(tx.scheduled_date)}</span>
                                                        </div>
                                                        ${tx.days_until > 0 ? `<span class="text-xs text-gray-500">in ${tx.days_until} days</span>` : ''}
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <div class="text-sm font-medium text-gray-900 dark:text-white">${tx.description}</div>
                                                        ${tx.merchant ? `<div class="text-xs text-gray-500">${tx.merchant}</div>` : ''}
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        ${tx.category ? `
                                                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium" style="background-color: ${tx.category.color}20; color: ${tx.category.color}">
                                                                <i class="${tx.category.icon} mr-1"></i>
                                                                ${tx.category.name}
                                                            </span>
                                                        ` : '<span class="text-gray-400 text-sm">-</span>'}
                                                    </td>
                                                    <td class="px-6 py-4 text-right">
                                                        <span class="text-sm font-bold ${tx.type === 'debit' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}">
                                                            ${tx.type === 'debit' ? '-' : '+'}${currencySymbol}${parseFloat(tx.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                        </span>
                                                    </td>
                                                    <td class="px-6 py-4 text-center">
                                                    ...
                                                    <td class="px-6 py-4 text-center">
                                                        <div class="flex items-center justify-center gap-2">
                                                            <button onclick="executeScheduledTransaction(${tx.id})" title="Execute Now"
                                                                class="p-2 text-green-600 hover:bg-green-100 dark:hover:bg-green-900/20 rounded-lg transition-colors">
                                                                <i class="fas fa-check"></i>
                                                            </button>
                                                            <button onclick="cancelScheduledTransaction(${tx.id})" title="Cancel"
            ...
            // Mobile cards
            mobileContainer.innerHTML = pendingTransactions.map(tx => {
                const isOverdue = tx.is_overdue;
                const isDueToday = tx.is_due_today;
                const statusClass = isOverdue ? 'bg-red-100 text-red-600' : isDueToday ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600';
                const statusText = isOverdue ? 'Overdue' : isDueToday ? 'Due Today' : 'Pending';

                return `
                                                <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-amber-200/50 dark:border-amber-700/30 shadow-sm">
                                                    ...
                                                    <div class="flex items-center justify-between">
                                                        <span class="text-lg font-bold ${tx.type === 'debit' ? 'text-red-600' : 'text-green-600'}">
                                                            ${tx.type === 'debit' ? '-' : '+'}${currencySymbol}${parseFloat(tx.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                                        </span>
                                                        <div class="flex items-center gap-2">
                                                            <button onclick="executeScheduledTransaction(${tx.id})"
                                                                class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200">
                                                                <i class="fas fa-check"></i>
                                                            </button>
                                                            <button onclick="cancelScheduledTransaction(${tx.id})"
        ...
        document.getElementById('scheduleForm').addEventListener('submit', async function (e) {
            ...
            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.show('Scheduling Transaction...');
            }

            const formData = {
                type: document.querySelector('input[name="schedule_type"]:checked').value,
                scheduled_date: document.getElementById('scheduleDate').value,
                amount: document.getElementById('scheduleAmount').value,
                description: document.getElementById('scheduleDescription').value,
                category_id: document.getElementById('scheduleCategory').value || null,
                merchant: document.getElementById('scheduleMerchant').value || null,
                notes: document.getElementById('scheduleNotes').value || null
            };

            try {
                const response = await fetch('/transactions/scheduled/store', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                ...
                if (response.ok && data.success) {
                    ...
                    if (typeof DynamicIsland !== 'undefined') {
                        DynamicIsland.success('Transaction Scheduled!');
                    }

                    popupSuccess(data.message || 'Transaction scheduled successfully!', 'Success');

                    setTimeout(() => {
                        closeScheduleModal();
                        btn.disabled = false;
                        icon.className = 'fas fa-calendar-check';
                        text.textContent = 'Schedule Transaction';
                        fetchScheduledTransactions();
                    }, 1000);
                } else {
                    throw new Error(data.message || 'Failed to schedule transaction');
                }
            } catch (error) {
                btn.disabled = false;
                icon.className = 'fas fa-calendar-check';
                ...
                popupError(error.message || 'An error occurred', 'Error');
            }
        });

        // Execute Scheduled Transaction
        async function executeScheduledTransaction(id) {
            const confirmed = await popupConfirm(
                'Execute this scheduled transaction now? This will create an actual transaction with today\'s date.',
                'Execute Transaction'
            );
            ...
            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.show('Executing Transaction...');
            }

            try {
                const response = await fetch(`/transactions/scheduled/${id}/execute`, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                if (response.ok && data.success) {
                    if (typeof DynamicIsland !== 'undefined') {
                        DynamicIsland.success('Transaction Executed!');
                    }
                    popupSuccess('Transaction executed successfully!', 'Success');
                    fetchScheduledTransactions();
                    fetchTransactions();
                } else {
        ...
        async function cancelScheduledTransaction(id) {
            const confirmed = await popupConfirm(
            ...
            if (typeof DynamicIsland !== 'undefined') {
                DynamicIsland.show('Cancelling...');
            }

            try {
                const response = await fetch(`/transactions/scheduled/${id}/cancel`, {
                    method: 'DELETE',
                    headers: {
                        'Accept': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                ...
                if (response.ok && data.success) {
                    if (typeof DynamicIsland !== 'undefined') {
                        DynamicIsland.success('Cancelled!');
                    }
                    popupSuccess('Scheduled transaction cancelled!', 'Success');
                    fetchScheduledTransactions();
                } else {
        ...
        // Load scheduled transactions on page load
        document.addEventListener('DOMContentLoaded', function () {
            fetchScheduledTransactions();
        });
    </script>
